...
function fe(a) {
  const t = I(), e = document.createElement("style");
  ...
  n.appendChild(h);
  const c = (i, s, d) => {
    const f = d.stringify, u = document.createElement("div");
    h.appendChild(u);
    const R = i, y = t[i], b = document.createElement("label");
    if (u.appendChild(b), b.innerText = d.label, b.setAttribute("for", R), b.style.width = "80px", b.style.display = "inline-block", b.style.textDecoration = "dotted underline 1px", b.title = d.title, s.type !== "checkbox") {
    ...
    u.appendChild(o), Object.assign(o, s), o.id = R, o.title = d.title, o.value = s.value !== void 0 ? s.value : y;
    const k = (p) => {
      t[i] = p, D(p);
    };
    o.oninput = () => {
    ...
    const B = (p) => {
      let w = p(t[i]);
      d.fixRange && (w = d.fixRange(w), o.max = Math.max(parseFloat(o.max), w), o.min = Math.min(parseFloat(o.min), w)), o.value = w, k(w);
    };
  ...
  c(
    ...
    { type: "range", min: 1 / 180 * Math.PI, max: 120.1 / 180 * Math.PI, step: 1 / 180 * Math.PI },
    {
      ...
      stringify: (i) => {
        const s = i / Math.PI * 180, d = Math.atan(Math.tan(i / 2) * t.aspect) * 2 / Math.PI * 180;
        return `${s.toFixed()}&deg;&times;${d.toFixed()}&deg;`;
      }
    }
  ), c(
    "depthiness",
    ...
    setTargetDiam((s) => {
      const f = Math.log(s) / Math.log(1.1);
      return Math.pow(1.1, f + i.deltaY * 0.01);
    });
  }), a.addEventListener("mousemove", (i) => {
    const s = i.movementX, d = -i.movementY;
    if (i.buttons & 2 || i.buttons & 1 && (i.shiftKey || i.ctrlKey)) {
      const f = t.trackballX, u = t.trackballY, R = -Math.cos(f) * s + Math.sin(f) * Math.sin(u) * d, y = -Math.cos(u) * d, b = Math.sin(f) * s + Math.cos(f) * Math.sin(u) * d;
      M((o) => o + R * t.targetDiam * 1e-3), A((o) => o + y * t.targetDiam * 1e-3), P((o) => o + b * t.targetDiam * 1e-3);
    } else
      i.buttons & 1 && (S((f) => f - s * 0.01), T((f) => f - d * 0.01));
  });
  ...
  function v() {
    ...
    const d = t.trackballX, f = t.trackballY, u = Math.cos(d) * i - Math.sin(d) * Math.cos(f) * s, R = -Math.sin(f) * s, y = -Math.sin(d) * i - Math.cos(d) * Math.cos(f) * s;
    M((b) => b + u * t.targetDiam * 0.03), A((b) => b + R * t.targetDiam * 0.03), P((b) => b + y * t.targetDiam * 0.03), requestAnimationFrame(v);
  }
  return n;
}
...
class me extends oe {
  constructor(t, e, n) {
    ...
    const l = fe(r), h = I(), c = this[ce].config, S = e.createTexture();
    let T, M;
    const A = e.createFramebuffer(), P = e.enable.bind(e), m = e.disable.bind(e), v = e.getExtension("OES_vertex_array_object"), i = 34229, s = v ? v.bindVertexArrayOES.bind(v) : e.bindVertexArray.bind(e), d = () => {
      const x = e.getParameter(e.TEXTURE_BINDING_2D);
      if (e.bindTexture(e.TEXTURE_2D, S), e.texImage2D(
        e.TEXTURE_2D,
        0,
        e.RGBA,
        h.framebufferWidth,
    ...
    (c.depth || c.stencil) && (c.depth && c.stencil ? M = { format: e.DEPTH_STENCIL, attachment: e.DEPTH_STENCIL_ATTACHMENT } : c.depth ? M = { format: e.DEPTH_COMPONENT16, attachment: e.DEPTH_ATTACHMENT } : c.stencil && (M = { format: e.STENCIL_INDEX8, attachment: e.STENCIL_ATTACHMENT }), T = e.createRenderbuffer()), d(), h.addEventListener("on-config-changed", d);
    const f = e.getParameter(e.FRAMEBUFFER_BINDING);
    e.bindFramebuffer(e.FRAMEBUFFER, A), e.framebufferTexture2D(e.FRAMEBUFFER, e.COLOR_ATTACHMENT0, e.TEXTURE_2D, S, 0), (c.depth || c.stencil) && e.framebufferRenderbuffer(e.FRAMEBUFFER, M.attachment, e.RENDERBUFFER, T), e.bindFramebuffer(e.FRAMEBUFFER, f);
    ...
    h.addEventListener("on-config-changed", B);
    ...
    const j = () => {
      if (!this[C].LookingGlassEnabled)
        return;
      (L.width !== h.calibration.screenW.value || L.height !== h.calibration.screenH.value) && (X = L.width, W = L.height, L.width = h.calibration.screenW.value, L.height = h.calibration.screenH.value);
      const x = e.getParameter(i), F = e.getParameter(e.CULL_FACE), V = e.getParameter(e.BLEND), z = e.getParameter(e.DEPTH_TEST), K = e.getParameter(e.STENCIL_TEST), Z = e.getParameter(e.SCISSOR_TEST), $ = e.getParameter(e.VIEWPORT), J = e.getParameter(e.FRAMEBUFFER_BINDING), Q = e.getParameter(e.RENDERBUFFER_BINDING), ee = e.getParameter(e.CURRENT_PROGRAM), te = e.getParameter(e.ACTIVE_TEXTURE);
      {
        ...
        e.bindFramebuffer(e.FRAMEBUFFER, null), e.useProgram(u), s(D), e.activeTexture(e.TEXTURE0), e.bindTexture(e.TEXTURE_2D, S), e.disable(e.BLEND), e.disable(e.CULL_FACE), e.disable(e.DEPTH_TEST), e.disable(e.STENCIL_TEST), e.viewport(0, 0, e.drawingBufferWidth, e.drawingBufferHeight), e.uniform1i(k, 0), e.drawArrays(e.TRIANGLES, 0, 6), g.clearRect(0, 0, r.width, r.height), g.drawImage(L, 0, 0), h.inlineView !== 0 && (e.uniform1i(k, h.inlineView), e.drawArrays(e.TRIANGLES, 0, 6)), e.bindTexture(e.TEXTURE_2D, ie);
      }
    ...
    const q = (x, F) => {
      !!_ != x && (x ? (B(), r.style.position = "fixed", r.style.top = "0", r.style.left = "0", r.style.width = "100%", r.style.height = "100%", r.width = h.calibration.screenW.value, r.height = h.calibration.screenH.value, document.body.appendChild(l), _ = window.open("", void 0, "width=640,height=360"), _.document.title = "Looking Glass Window (fullscreen me on Looking Glass!)", _.document.body.style.background = "black", _.document.body.appendChild(r), console.assert(F), _.onbeforeunload = F) : (l.parentElement.removeChild(l), L.width = X, L.height = W, _.onbeforeunload = void 0, _.close(), _ = void 0));
    };
    this[C] = {
...
class be extends re {
  ...
  onFrameStart(t, e) {
    const n = this.sessions.get(t), r = I();
    if (n.immersive) {
      const g = Math.tan(0.5 * r.fovy), l = 0.5 * r.targetDiam / g, h = l - r.targetDiam, c = this.basePoseMatrix;
      ...
      for (let T = 0; T < r.numViews; ++T) {
        const M = (T + 0.5) / r.numViews - 0.5, A = Math.tan(r.viewCone * M), P = l * A, m = this.LookingGlassInverseViewMatrices[T] = this.LookingGlassInverseViewMatrices[T] || E.create();
        E.translate(m, c, [P, 0, 0]), E.invert(m, m);
        const v = Math.max(h + e.depthNear, 0.01), i = h + e.depthFar, s = v * g, d = s, f = -s, u = v * -A, R = r.aspect * s, y = u + R, b = u - R, o = this.LookingGlassProjectionMatrices[T] = this.LookingGlassProjectionMatrices[T] || E.create();
        E.set(
          o,
          2 * v / (y - b),
          0,
  ...
  getViewport(t, e, n, r, g) {
    if (g === void 0) {
      ...
      const l = I(), h = g % l.quiltWidth, c = Math.floor(g / l.quiltWidth);
      r.x = l.tileWidth * h, r.y = l.tileHeight * c, r.width = l.tileWidth, r.height = l.tileHeight;
    }
    return !0;
  }
